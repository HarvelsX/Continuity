...
public class ContinuityConfigScreen extends Screen {
	...
	protected void init() {
		...
		addDrawableChild(ButtonWidget.builder(ScreenTexts.DONE, button -> close()).dimensions(width / 2 - 100, height - 40, 200, 20).build());
	}
	...
	private ButtonWidget createBooleanOptionButton(int x, int y, int width, int height, Option<Boolean> option) {
		...
		return ButtonWidget.builder(ScreenTexts.composeToggleText(text, option.get()), button -> {
